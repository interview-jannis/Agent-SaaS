...
2. **스케줄 작업** (awaiting_schedule 단계):
   ...
   - **Schedule Editor** — Day 카드 + Add Item form (block / time / product link / title / location / notes), 위/아래 화살표 정렬. Save = 새 schedules row + items JSONB + version bump + cases.status → reviewing_schedule. PDF 업로드 경로는 backward compat로만 (legacy view 가능, 신규 X). **5/14~15**: row 좌측 체크박스 제거(Transfer는 Add Item으로), Add Item 시 그룹 강제 선택(미선택 시 저장 차단), hotel sub-dropdown 3-enum (check-in/stay/check-out), hotel 박수 = day span (max-min+1, 호텔 product 기준), **day-level concierge subpackage** chip + per-day hours input (시간 단위 서비스만). Trip Services Summary 박스 추가(day/hours contracted 대비).
...
- **schedules**: id, case_id, quote_id(이름만 남음, FK drop됨, 실제로는 quotation document.id), slug, pdf_url(legacy/nullable), **items**(JSONB, ScheduleItem[]), status, version, created_at, file_name, revision_note, confirmed_at, **first_opened_at, open_count**, concierge_name, concierge_phone, **day_subpackages**(JSONB, `{ [day]: { variantId, hours }[] }` — day-level concierge subpackage 할당 + per-day hours; legacy `string[]` read-tolerated), UNIQUE(case_id, version). 신규는 items 사용, pdf_url은 backward compat로만 유지 (legacy 케이스 view).
...
커밋 시 소스 변경 파일 외에 **반드시** 아래 폴더도 함께 스테이징할 것:

```
git add data/ sql/
```

- `data/` — 상품 마스터 Excel, 파싱 스크립트, 텍스트 소스 파일 등
- `sql/` — 스키마 마이그레이션, 패치 SQL 파일 등

이 파일들은 `.gitignore` 제외 대상이 아니지만 `git add` 시 자동으로 올라가지 않음(untracked). 매 커밋마다 위 명령을 반드시 포함하여 누락 방지. 사용자가 별도로 요청하지 않아도 항상 실행.
