...
## 현재 상태
- **Phase**: 시뮬 D+5(5/16). Agent 카탈로그 잔손질 + 상품 description 정리본 + Hotel 큐레이션 + Row 3 통일.
- **마지막 작업**: 2026-05-16 — Subpackage Hotel region tertiary 도입, Row 3 pills 노출 통일 (subcategory 클릭 시에만), K-Medical/K-Wellness 정렬 룰 추가, 159개 description 정리본 텍스트 파일, Hotel 큐레이션 가이드 (37→31).
- **마지막 업데이트**: 2026-05-16 (시뮬레이션 5/11~15, 런칭 5/18)
...
> 2026-05-11 오후: `notes/26.05.11.md` §5–9 (#19 섹션 border-2+상태뱃지. Balance Invoice 자동발행 버그 4건. Schedule Editor Row UX. Free time/Prayer draft. 다중 그룹 배정).
...
- [x] **스케줄 자동 링크** (5/5) — 입력은 UI 직접 편집(Day 카드 + Add Item form), 출력은 Option A 에디토리얼 렌더. PDF 경로는 backward compat로만. `schedules.items` JSONB + `ScheduleDocument.tsx` + `admin/ScheduleEditor.tsx` + `src/types/schedule.ts`. Cover 자동 (lead client + dates + 호텔 + concierge). version 관리 기존 그대로. SQL 마이그레이션: `sql/2026-05-05_schedules_items.sql`. drag-drop 없이 위/아래 화살표 v1.
- [x] **항목 편집 단계 분리** (5/5) — quotation line item add/remove 권한이 awaiting_schedule 단계로 이동, awaiting_pricing은 가격만 (admin/cases/[id]에 신규 "Edit Selected Products" 섹션 + Finalize Pricing에서 picker/× 제거). save-on-action 모델로 staging 없음.
