...
const DISPLAY_LABELS: Record<DisplayGroup, string> = {
  awaiting_review: STATUS_LABELS.awaiting_review,
...
const DISPLAY_STYLES: Record<DisplayGroup, string> = {
  awaiting_review: STATUS_STYLES.awaiting_review,
...
export default function AgentCasesPage() {
  ...
  function getDisplayGroup(c: CaseRow): DisplayGroup {
    if (c.status === 'awaiting_review') return 'awaiting_review'
    if (c.status === 'completed' || c.status === 'awaiting_settlement') {
      ...
      <div className="shrink-0 border-b border-gray-100 px-4 md:px-6 py-3 flex flex-col gap-2.5">
        ...
        <div className="flex items-center gap-3">
          ...
          <input
            ...
            className="ml-auto flex-1 md:flex-none md:w-64 text-xs border border-gray-200 rounded-lg px-3 py-1.5 focus:outline-none focus:border-[#0f4c35] bg-white text-gray-900 placeholder-gray-400"
          />
        </div>
                    ...
                    return (
                      <Fragment key={group}>
                        <tr id={`status-section-${group}`} className="scroll-mt-20 bg-gray-100">
                          <td colSpan={6} className={`px-6 border-b border-gray-200 ${empty ? 'py-1' : 'py-2'}`}>
