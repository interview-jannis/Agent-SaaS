...
export default function AgentPayoutsPage() {
  ...
  const unsettled = cases.filter(c => (c.status === 'completed' || c.status === 'awaiting_review') && !settledCaseIds.has(c.id))
  ...
  return (
    <div className="flex flex-col h-full bg-white">
      <div className="h-14 shrink-0 flex items-center gap-4 px-6 border-b border-gray-100">
        <h1 className="text-base font-semibold text-gray-900">Payouts</h1>
      </div>

      <div className="flex-1 overflow-y-auto">
        <div className="px-4 md:px-12 py-6 md:py-8 space-y-8">
