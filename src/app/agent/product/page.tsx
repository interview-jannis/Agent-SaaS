...
export default function AgentProductPage() {
  ...
  // Any other entry to Product (from another tab, fresh login, etc.) starts with an empty cart.
  useEffect(() => {
  ...
  // Subcategories present within the currently selected category, ordered by sort_order.
  const availableSubcategories = useMemo(() => {
    if (!selectedCategoryId) return [] as string[]
    const productSubIds = new Set(
      products
        .filter(p => p.category_id === selectedCategoryId && p.subcategory_id)
              ...
              <div className="flex items-start justify-between gap-3">
                <div>
                  ...
                  {detailProduct.partner_name && (
                    <p className="text-xs text-gray-500 mt-0.5">{detailProduct.partner_name}</p>
                  )}
                </div>
                  ...
                  return (
                    <div className="flex items-center gap-4">
                      <div className="bg-gray-50 rounded-xl px-4 py-2.5 text-center">
                        ...
                        <p className="text-xs text-gray-400">{toUSD(detailProduct) === 'Price on request' ? 'contact us' : 'per person'}</p>
                      </div>
                      <div className="bg-gray-50 rounded-xl px-4 py-2.5 text-center">
