...
// Server-side 3-party contract counter-sign — admin side.
// Mirrors /api/admin/sign-contract: validates typed name vs admin.name,
// captures IP/UA, hashes signature image.
...
export async function POST(req: Request) {
  ...
  const { data: admin } = await supabase.from('admins').select('id, name, title, can_sign_contracts').eq('auth_user_id', uid).maybeSingle()
  if (!admin) return NextResponse.json({ error: 'Admin record not found.' }, { status: 403 })
  const adm = admin as { id: string; name: string; title: string | null; can_sign_contracts: boolean | null }
  ...
  if (typed.toLowerCase() !== adm.name.trim().toLowerCase()) {
    return NextResponse.json({ error: `Typed name does not match your registered name (${adm.name}).` }, { status: 400 })
  }

  const { data: contract } = await supabase.from('case_contracts')
    .select('id, admin_signed_at').eq('id', contract_id).maybeSingle()
